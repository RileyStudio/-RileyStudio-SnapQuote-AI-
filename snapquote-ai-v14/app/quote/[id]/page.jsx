    ...
    async function load() {
      if (supabase) {
        const { data, error } = await supabase
          .from('estimates')
          .select('*, jobs(*, contractors(*), job_photos(*))')
          .eq('public_token', id)
          .single();

        if (!error && data && isMounted) {
          setQuote(mapSupabaseEstimate(data));
          setIsDemo(false);
          setApproved(Boolean(data.approved_at));
...
// Maps a real Supabase row (estimates joined with jobs + contractors +
// job_photos) into the same shape demoQuote uses, so the JSX above never
// needs to know which source it's rendering.
function mapSupabaseEstimate(row) {
  const job = row.jobs;
  ...
  return {
    ...
    expiration_date: addDays(row.created_at, 14),
    ticket_number: row.id ? row.id.slice(0, 8).toUpperCase() : undefined,
    // TODO: storage_path needs a signed URL from a server route before
    // production use — Storage buckets are private. Captions are a
    // placeholder until photo metadata/captions are captured at upload time.
    photos: (job?.job_photos || []).map((p) => ({ id: p.id, caption: 'Job site photo' })),
    ...
    contractor: {
      ...
      initials: initialsOf(contractor?.business_name),
      brand_color: contractor?.brand_color || '#FF5A1F',
      phone: contractor?.phone,
      logo_url: contractor?.logo_url || undefined,
      license_note: contractor?.license_number ? `Lic. ${contractor.license_number}` : undefined,
    },
  };
}
