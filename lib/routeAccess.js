import { hasFeature, normalizePlan } from './plans';

// plan here is one of:
//   - a real plan key: 'solo' | 'founder' | 'pro' | 'teams' | 'admin'
...
export function canAccessRoute(plan, route) {
  ...
  // areas below. Any other value is normalized to a canonical key so a
  // legacy "team" still resolves to "teams".
  const effectivePlan = plan === 'demo' ? 'solo' : (plan ? normalizePlan(plan) : plan);
  ...
  if (route === '/team' || route.startsWith('/team/')) {
    return effectivePlan === 'teams'; // Teams only
  }
