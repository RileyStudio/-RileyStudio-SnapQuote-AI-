...
import { generateQuotePdf, downloadBlobFile, downloadHtmlFile, sendEstimateEmail } from '@/lib/apiClient';
...
export default function EstimateReviewPage({ params }) {
  ...
  async function handleSendEmail() {
    if (!estimate.customer?.email) {
      setEmailMessage('Add a customer email address to send this estimate.');
      return;
    }
    setEmailing(true);
    setEmailMessage('');
    try {
      const link = sentLink
        ? `${typeof window !== 'undefined' ? window.location.origin : ''}${sentLink}`
        : '';
      const result = await sendEstimateEmail({
        ...
        quoteLink: link,
        type: estimate.sentAt ? 'resend' : 'send',
      });
      setEmailMessage(result.demo ? result.message : 'Email sent.');
    } catch (e) {
          ...
          <div className="text-center bg-approved/10 rounded-card p-5">
            ...
            <div className="mt-4 space-y-2">
              ...
              {hasFeature(plan, 'notifications') ? (
                <BigButton variant="ghost" onClick={handleSendEmail} disabled={emailing}>
                  {emailing ? 'Sending…' : estimate.sentAt ? 'Resend Quote Email' : 'Send Estimate Email'}
                </BigButton>
              ) : (
                <p className="text-xs text-ink/45">
                  Email notifications are available on the {planLabel('pro')} plan.
                </p>
              )}
            </div>
            {emailMessage && <p className="mt-3 text-sm text-ink/60">{emailMessage}</p>}
          </div>
        ) : (
          <>
