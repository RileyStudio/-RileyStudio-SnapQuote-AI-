...
- **Email sending now has a real implementation (Resend), but it's
  demo-mode by default.** With no `RESEND_API_KEY` configured, "Send
  Estimate Email" and "Resend Quote" return a clear placeholder instead of
  actually emailing anyone — see `app/api/send-email/route.js`.
- **Payment/billing is not implemented.** No Stripe, no deposit collection,
  no invoicing beyond the quote/estimate itself. The Solo/Pro/Team plans
  in Settings are feature gates only (`lib/plans.js`) — switching plans is
  instant and free, by design; there's no checkout flow to gate behind it.
...
| `RESEND_API_KEY` | `/api/send-email` | Returns a demo placeholder (`demo: true`, nothing actually sent) instead of erroring |
| `SEND_EMAIL_FROM` | `/api/send-email` (optional) | Defaults to `estimates@snapquoteai.app` — must be a domain verified with your email provider in production |
