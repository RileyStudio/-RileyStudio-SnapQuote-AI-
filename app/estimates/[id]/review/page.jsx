...
export default function EstimateReviewPage({ params }) {
  ...
  // Safe to read here (not in an effect): this whole branch only ever
  // executes after the estimate-loading effect above has already run, so
  // we're guaranteed to be client-side already — no hydration mismatch risk.
  const settings = getSettings();
  const previewContractor = contractorFromSettings(settings);
  ...
  function sendToCustomer() {
    setSending(true);
    setTimeout(() => {
      const updated = markEstimateSent(estimate.id);
  ...
  return (
    <main className="max-w-2xl mx-auto px-5 py-6 pb-10">
      ...
      <JobTicketCard
        ...
        expirationDate={formatDate(addDays(estimate.createdAt, settings.estimateTerms.expiration_days))}
